...
- **Task shape: no longer contested. Shape is real and large, AND code specifically is a conjunction.** I previously left this open because one stack measured coding-shaped prompts reasoning **least** and another **most**. His grid crossed four shapes against all ten apparatus levels, so it can separate the two axes, and the answer has two parts.
